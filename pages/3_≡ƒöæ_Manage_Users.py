...
with tab_add:
    ...
    with st.form("add_user_form", clear_on_submit=True):
        ...
        role = st.selectbox("Role*", ["school", "admin"], format_func=lambda r: "School-level" if r == "school" else "Admin")
        school_choice = None
        if role == "school":
    ...
    if submitted:
        if not (full_name and username and password):
        ...
        elif role == "school" and not school_choice:
            ...
            if role == "school":
                ...
                school_id = match["id"]
            try:
                db.create_user(username.strip(), password, full_name, role, school_id, created_by=current_admin)
                st.success(f"Created {role} account for {full_name}.")
